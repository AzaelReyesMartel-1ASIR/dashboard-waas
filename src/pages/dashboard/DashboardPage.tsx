...
export default function DashboardPage() {
  ...
  return (
    <div className="space-y-8">
      <div>
        <h1 className="text-3xl font-bold text-white tracking-tight">Gestor de Enlaces</h1>
      ...
      {/* Input Form */}
      <div className="bg-slate-900/50 border border-slate-800 rounded-2xl p-6 backdrop-blur-xl relative">
        <h2 className="text-lg font-bold text-white mb-4">Añadir nuevo enlace</h2>
        ...
        <form onSubmit={handleCreate} className="grid grid-cols-1 md:grid-cols-3 gap-4 items-end">
          <div>
            ...
            <input
              ...
              required
              className="w-full px-4 py-3 rounded-xl bg-slate-950/80 border border-slate-800 text-white placeholder-slate-600 focus:outline-none focus:ring-2 focus:ring-indigo-500/40 focus:border-indigo-500 transition-all duration-200 text-sm"
            />
          </div>

          <div>
            <label htmlFor="url" className="block text-xs font-semibold uppercase tracking-wider text-slate-400 mb-2">
              URL de Destino
            </label>
            <input
              id="url"
      ...
      <div className="bg-slate-900/50 border border-slate-800 rounded-2xl overflow-hidden backdrop-blur-xl">
        <div className="px-6 py-4 border-b border-slate-800 flex justify-between items-center">
          <h2 className="text-lg font-bold text-white">Enlaces Activos</h2>
        ...
        {loading ? (
          ...
          <div className="p-12 text-center text-slate-500">
            ...
            <p className="text-sm">No tienes enlaces creados todavía.</p>
            <p className="text-xs text-slate-600 mt-1">Completa el formulario superior para añadir uno.</p>
          </div>
        ) : (
          <div className="overflow-x-auto">
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="border-b border-slate-800/80 text-xs font-semibold uppercase tracking-wider text-slate-400 bg-slate-950/20">
                ...
                {links.map((link, index) => (
                  ...
                  >
                    ...
                    <td className="py-4 px-6 text-sm text-slate-500">
                      <DynamicIcon name={link.icon} className="w-5 h-5 text-gray-400" />
                    </td>
                    <td className="py-4 px-6 text-right">
                      <button
                        onClick={() => setLinkToDelete(link.id)}
                        className="px-3 py-1.5 rounded-lg text-xs font-semibold text-red-400 hover:text-red-300 hover:bg-red-500/10 border border-transparent hover:border-red-500/20 transition-all duration-200"
                      >
                        Eliminar
                      </button>
                    </td>
      ...
      {linkToDelete !== null && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center backdrop-blur-sm px-4 animate-fade-in">
          <div className="bg-gray-900 border border-gray-800 rounded-lg p-6 w-full max-w-sm shadow-2xl space-y-4">
            <div>
              <h3 className="text-lg font-bold text-white">¿Eliminar enlace permanente?</h3>
            ...
            <div className="flex justify-end gap-3 pt-2">
              <button
                ...
                disabled={isDeleting}
                className="px-4 py-2 rounded-lg text-sm font-medium text-slate-400 hover:text-white bg-slate-800 hover:bg-slate-700 transition-colors duration-150 disabled:opacity-50"
              >
                Cancelar
              </button>
              <button
